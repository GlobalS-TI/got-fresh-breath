...
export async function SectoresGrid() {
  const sectores = await Promise.all(
    SECTORES.map(async (sector) => ({ ...sector, media: await getSiteMediaSlot(`sectores.${sector.value}`) })),
  )
  ...
  return (
    <section>
      ...
      <div className="grid gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
        {sectores.map((sector) => (
          <div key={sector.value} className="flex flex-col">
            <div className="overflow-hidden rounded-md border border-slate-200">
              <MediaSlot
                media={sector.media}
                fallbackLabel={`Foto - ${sector.label} (pendiente del cliente)`}
                className="flex h-44 items-center justify-center border border-dashed border-slate-300 bg-slate-100 px-3 text-center text-xs text-slate-400"
                mediaClassName="h-44 w-full object-cover"
              />
